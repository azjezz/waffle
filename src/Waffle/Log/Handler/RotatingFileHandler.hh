...
use type Waffle\Log\Exception\InvalidArgumentException;
...
class RotatingFileHandler extends StreamHandler
{
     ...
     */
    <<__Override>>
    public function __construct(string $filename, int $maxFiles = 0, LogLevel $level = LogLevel::DEBUG, bool $bubble = true, ?int $filePermission = null, bool $useLocking = false)
    {
    ...
    private function rotate(): void
    {
        ...
        $logFiles = Dict\take(
            Dict\reverse(Dict\sort($logFiles, (string $a, string $b): int ==> Str\compare($b, $a))),
            $this->maxFiles
        );
    ...
    private function getTimedFilename(): string
    {
        ...
        if (C\contains_key($fileInfo, 'extension') && '' !== $fileInfo['extension']) {
