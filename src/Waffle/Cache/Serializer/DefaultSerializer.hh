...
class DefaultSerializer implements SerializerInterface
{
    ...
    public function unserialize(string $value): mixed
    {
        ...
        try {
            ...
            $message = (false === $error) || ($error['message'] === null) ? 'Failed to unserialize values' : $error['message'] as string;
            throw new DomainException($message);
        } catch(Error $e) {
            throw new ErrorException($e->getMessage(), (int) $e->getCode(), E_ERROR, $e->getFile(), $e->getLine());
        }
    }
}
